...
def wait_for_backend(max_seconds: int = 90):
    ...
    try:
        health = _call("get", "/health", timeout=60)
        placeholder.empty()
        return health
    ...
    while time.time() < deadline:
        time.sleep(delay)
        try:
            health = _call("get", "/health", timeout=30)
            placeholder.empty()
            return health
        except BackendUnavailable:
...
# ---------------------------------------------------------------------------
with tab_models:
    metrics = api_get("/metrics") if health else None
    if metrics:
...
with tab_chat:
    ...
    if st.button("Ask", type="primary") and q:
        ...
        if res:
            ...
            st.info("The service is still waking up. Try that question again in a moment.")
